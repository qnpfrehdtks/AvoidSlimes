...
public class Boss5PatternFactory : PatternFactory
{


    public Boss5PatternFactory(BOSS boss) : base(boss)
    {
    ...
    protected override void createPattern()
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            for (int j = 0; j < 2; j++)
            {
                ...
                newPattern.setPattern
                     ...
                     9,                            //    int     ||     Count        || 탄환을 한번에 발사 하는 놈의 수를 조정.
        ...
        for (int i = 0; i < 2; i++)
        {
            ...
            newPattern.setPattern
                ...
                0.0f,                        //    float   ||  BulletSpeedRate || 탄환이 가속도  
                42f - 2.0f * i,                        //    float   ||  BulletAngleRate || 탄환의 각 증가속도?? 알아서 탄환의 각이 커짐. 
                20,                           //    int    ||     Count        || 탄환을 한번에 발사 하는 놈의 수를 조정.
                0.1f,                         //    float  ||  MinInterval     || ShotGun ,Direct 패턴에서 탄환의 그룹안의 탄환간의 간격
                0.0f,                        //    float   ||   RotateAngle     || 거의 쓸일은 없겠지만, 회전 각 정하기? Washer에서 주로 쓰임.
        ...
        newPattern.setPattern
            ...
            0.4f,                        //    float   ||    Interval      || 탄환 사이의 간격 (시간)
            0.0f,                        //    float   ||  BulletSpeedRate || 탄환이 가속도  
            38.0f,                        //    float   ||  BulletAngleRate || 탄환의 각 증가속도?? 알아서 탄환의 각이 커짐. 
            20,                           //    int    ||     Count        || 탄환을 한번에 발사 하는 놈의 수를 조정.
            0.1f,                         //    float  ||  MinInterval     || ShotGun ,Direct 패턴에서 탄환의 그룹안의 탄환간의 간격
            0.0f,                        //    float   ||   RotateAngle     || 거의 쓸일은 없겠지만, 회전 각 정하기? Washer에서 주로 쓰임.
            10.00f,                       //    float  ||  RotateAngleRate || 탄환이 회전 하는 패턴을 가질 경우, 회전 각 속도 조정 Rotate 붙은 패턴에서 쓰임.
        ...
        for (int i = 0; i < 5; i++)
        {
            ...
            newPattern.setPattern
                ...
                7.00f,                       //    float   ||  RotateAngleRate || 탄환이 회전 하는 패턴을 가질 경우, 회전 각 속도 조정 Rotate 붙은 패턴에서 쓰임.
